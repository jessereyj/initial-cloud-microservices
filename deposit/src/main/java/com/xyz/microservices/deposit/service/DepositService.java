package com.xyz.microservices.deposit.service;
...
public class DepositService {
	...
	public DepositService( CircuitBreakerFactory  cbFactory) {
		this.cbFactory = cbFactory;
		httpHeaders = new HttpHeaders();
		httpHeaders.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
		circuitBreaker = this.cbFactory.create("slow");
	}

	public ResponseEntity<ApiResponse> cashDeposit(HttpServletRequest request, CashDepositRequest CashDepositRequest) {
		String token = HttpUtil.getJWTToken(request, CashDepositRequest.getJwt()); 
		...
		return circuitBreaker.run(() -> 
		  	  this.submitDeposit(apiConfig.getBaseUrl().concat(apiConfig.getCashDepositPath()), new HttpEntity<>(CashDepositRequest, httpHeaders)),
	...
	public ResponseEntity<ApiResponse> submitDeposit(String apiUrl, HttpEntity<?> httpEntity)  {
		...
		try {
			...
			if (!StringUtils.isEmpty(e.getResponseBodyAsString())) {// Validation Error
				...
				ApiResponse.setMessage(StringUtils.defaultIfEmpty(e.getStatusText(),StringUtils.EMPTY).concat(StringUtils.EMPTY).concat(StringUtils.defaultIfEmpty(e.getMessage(),StringUtils.EMPTY)));
